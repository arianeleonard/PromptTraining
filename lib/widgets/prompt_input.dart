...
/// User input area with rich interaction features
class PromptInput extends StatelessWidget {
  final Widget child;
  final void Function()? onSubmit;

  const PromptInput({super.key, required this.child, this.onSubmit});
...
class PromptInputToolbar extends StatelessWidget {
  ...
  const PromptInputToolbar({super.key, required this.child});
...
class PromptInputTools extends StatelessWidget {
  ...
  const PromptInputTools({super.key, required this.children});
...
class _PromptInputModelSelectState extends State<PromptInputModelSelect> {
  ...
  OverlayEntry _createOverlayEntry() {
    ...
    final dropdownHeight =
        idealHeight > maxDropdownHeight ? maxDropdownHeight : idealHeight;
...
class PromptInputComplete extends StatefulWidget {
  final void Function(String prompt, String modelId, String? context)? onSubmit;
  final List<String>? models;
  ...
  void _handleSubmit() {
    ...
    if (text.isNotEmpty && widget.status != ChatStatus.submitting) {
      ...
      try {
        ...
        chat.setInputDraft('', focus: false);
      } catch (_) {}
      if (_showContext) {
        setState(() {
          _showContext = false;
  ...
  Widget build(BuildContext context) {
    return Consumer<ChatProvider>(
      builder: (context, chatProvider, _) {
        // Inject draft text only on new draft events
        if (chatProvider.draftVersion != _lastDraftVersion) {
          _controller.text = chatProvider.inputDraft;
          _controller.selection = TextSelection.fromPosition(
            TextPosition(offset: _controller.text.length),
          );
          _lastDraftVersion = chatProvider.draftVersion;
          // No setState() here: we're already in build due to provider notify.
          // The updated controller text is used in this same build pass.
        }

        // Focus input when a new focus request arrives
        if (chatProvider.focusTick != _lastFocusTick) {
          // Defer focus to the end of this frame to avoid markNeedsBuild during build
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (!mounted) return;
            _promptFocusNode.requestFocus();
          });
          _lastFocusTick = chatProvider.focusTick;
        }
