...
class OpenRouterService {
    ...
    try {
      final response = await http
          .post(
            ...
            body: jsonEncode({
              'model': model,
              'messages': _buildApiMessages(messages, systemPrompt),
              'temperature': 0.7,
              'max_tokens': 2048,
            }),
          )
  ...
  /// Build the API messages array, optionally prepending a system prompt.
  List<Map<String, dynamic>> _buildApiMessages(
    ...
    list.addAll(messages.map(_messageToJson));
    return list;
  }
